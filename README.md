...
Google Fonts의 Noto Sans KR을 사용하며, 연결되지 않으면 시스템 sans-serif 폰트로 표시됩니다. 홈 인트로는 사용자가 제공한 이미지를 사용합니다. 제품 이미지 자리는 준비 중입니다. 블로그는 JSON 파일을 수정하여 발행하는 방식이며 관리자 화면이나 자동 발행 서비스는 포함하지 않습니다.

...
function framedPhoto(product,url){
 ...
 if(f.outline){const defs=document.createElementNS(ns,'defs'),clip=document.createElementNS(ns,'clipPath'),poly=document.createElementNS(ns,'polygon');clip.id=`photo-${product.id}`;poly.setAttribute('points',f.outline);clip.append(poly);defs.append(clip);svg.append(defs);image.setAttribute('clip-path',`url(#${clip.id})`);}
